...
try:
        ...
        # print 'module pywt is not installed, wavelet smoothing capabilities
        # are disabled'
        pass
...
class FID_array(object):
        ...
        def procpar(self, value):
                if not value:
                        raise ValueError(
                            'Parameter dictionary cannot be empty.')
                if not isinstance(value, dict):
                        raise ValueError(
                            'Paramters must be in a dictionary object, not type %s.' %
                            type(value).__name__)
                self._procpar = value
        ...
        def _f_extract_proc_varian(self):
                ...
                rt = at+d1
                nt = np.array(
                    [self._procpar['procpar']['nt']['values']], dtype=float)
                acqtime = (nt*rt).cumsum()/60.  # convert to mins.
                sw = round(
                    float(self._procpar['procpar']['sw']['values'][0]) /
                    float(self._procpar['procpar']['sfrq']['values'][0]), 2)
                ...
                if len(self.data.shape) == 2:
                        for i in self.data:
                                dz.append(
                                    i.tolist() + np.zeros(2
                                                          **
                                                          (1 + np.floor(np.log(len(i)) / np.log(2))) -
                                                          len(i)).tolist())   # _like(i).tolist())
                self.data = np.array(dz)
        ...
        def emhz(self, lb=10.0):
                ...
                """
                self.data = np.exp(-np.pi *
                                   np.arange(len(self.data[0]))*(lb/self.params['sw_hz']))*self.data
        ...
        def loadfids(self, filename=None):
                ...
                if np.isnan(new_array[1]):
                        ...
                        self.data = new_array[
                            2:].reshape(
                            [int(new_array[0]), int(new_array[1])])
        ...
        def plot_array_ppm(
            ...
            y_space=None,
                ...
                acqtime = np.array(
                    [self.params['acqtime'][0]] * len(data_ft1)).cumsum()
                acqtime -= acqtime[0]
                sw = [sw_left, sw_left-self.params['sw']]
                if left > sw[0]:
                        return 'Up-field limit of ' + \
                            str(left)+' exceeds spectral width of '+str(sw[0])
                if right < sw[1]:
                        return 'Down-field limit of ' + \
                            str(right)+' exceeds spectral width of '+str(sw[1])
                sw = sw[::-1]
                ...
                if labels is not None:
                        for i in labels:
                                if ppm[0] > labels[i] > ppm[-1]:
                                        xlbl = np.where(
                                            abs(labels[i] - ppm) ==
                                            abs(labels[i] - ppm).min())[0][0]
                                        ax_all.plot(
                                            (xlbl, xlbl),
                                            (acqtime[-1], acqtime[-1]),
                                            (data_ft[-1][xlbl] + 0.1,
                                             data_ft[-1][xlbl] + 0.2), 'k',
                                            linewidth=1)
                                        ax_all.text(
                                            xlbl - 0.009 * abs(right - left),
                                            acqtime[-1], data_ft[-1][xlbl] +
                                            0.2 * max(data_ft[-1]), i, zdir=(1,
                                                                             0, 0), rotation_mode='anchor',
                                            fontsize=fs)
                #----------------------------
                ...
                if np.ceil(plotrange[0]) == plotrange[0]:
                        nlbls = np.arange(
                            np.ceil(plotrange[1]), np.ceil(plotrange[0])+1)[::-1]  # [1:-1]
                else:
                        nlbls = np.arange(
                            np.ceil(plotrange[1]), np.ceil(plotrange[0]))[
                            :: -1]   # [1:-1]
                lbl_ind = []
                for i in nlbls:
                        lbl_ind.append(
                            np.where(abs(ppm - i) == abs(ppm - i).min())[0][0])
                ...
                if len(nlbls) > 2*maxlbls:
                        ind_red = np.array(
                            np.mgrid[
                                ...
                                1:complex(10)],
                            dtype=int)
                        # np.array([0,int(np.ceil(len(nlbls)/2)),-1])]
                        nlbls = nlbls[ind_red]
                        # np.array([0,int(np.ceil(len(nlbls)/2)),-1])]
                        lbl_ind = lbl_ind[ind_red]
                ...
                if len(nlbls) < minlbls:
                        nlbls = np.array(
                            [plotrange[0], np.mean(plotrange), plotrange[1]])
                ...
                ytcks = np.array(
                    [acqtime[0], acqtime[data_ft.shape[0] - 1] / 2,
                     acqtime[data_ft.shape[0] - 1]])
                ylbls = []
                for i in ytcks:
                        ylbls.append(str(np.round(i, 0)))
                ...
                for i in zip(xts[0], xts[1]):
                        ax_all.plot(
                            (i[1], i[1]), (acqtime[0], acqtime[0]),
                            (-0.01, -0.05), 'k', linewidth=1)
                        ax_all.text(
                            i[1], -0.2 * acqtime[-1], -0.2 * ax_all.get_ylim
                            ()[-1], i[0], zdir=(1, 0, 0), ha='center',
                            rotation_mode='anchor', fontsize=10)

                yts = acqtime[np.array([0, -1])].round(1)
                for i in yts:
                        ax_all.text(-0.05*data_ft.shape[1],
                                    i,
                                    0,
                                    i,
                                    zdir=(1,
                ...
                ax_all.text(-0.05*data_ft.shape[1],
                            ...
                            fontsize=10)
                if y_space is None:
                        y_space = -0.4*acqtime[-1]
                ax_all.text(
                    0.5 * data_ft.shape[1], y_space, 0, x_label, zdir=(1, 0, 0),
                    ha='center', rotation_mode='anchor', fontsize=10)
                ax_all.set_xlim3d(x[0], x[-1])
                ax_all.set_ylim3d(0, acqtime[data_ft.shape[0]-1])
                ax_all.set_zlim3d(0, (1/amp)*data_ft.max())
                ax_all.set_axis_off()
                if filename is not None:
                        fig_all.savefig(filename, format='pdf')
                show()
        ...
        def plot_fid(
                ...
                if labels == 'peaks':
                        if self.peaks:
                                ...
                                labels = dict(
                                    zip([str(i) for i in peaks], peaks))
                        else:
                                labels = {}
                if not sw_left:
                        sw_left = self.params['sw_left']
                ...
                if len(self.data.shape) == 2:
                        ppm = np.mgrid[
                            sw_left - self.params['sw']: sw_left:
                            complex(len(self.data[0]))]
                ...
                if labels:
                        ...
                        for i in labels:
                                if ppm[0] < labels[i] < ppm[-1]:
                                        xlbl = len(
                                            self.data[label_index])-np.where(abs(labels[i]-ppm) == abs(labels[i]-ppm).min())[0][0]
                                        ax1.plot(
                                            (labels[i], labels[i]),
                                            (self.data[label_index][xlbl] +
                                             label_distance_frac / 3.0 *
                                             lbl_gap, self.data
                                             [label_index][xlbl] + 2 / 3. *
                                             label_distance_frac * lbl_gap),
                                            color='0.5')
                                        ax1.text(
                                            labels[i],
                                            self.data[label_index][xlbl] +
        ...
        def phasespace(self, index=0, inc=50, lims=180):
                ...
                phases = np.mgrid[
                    lims[0]:lims[1]:complex(inc),
                    lims[0]:lims[1]:complex(inc)]
                phases = np.transpose(
                    [phases[0].flatten(), phases[1].flatten()])
                d_p = np.array(
                    [sum
                     (abs(self.ps(data, i[0], i[1]).real))
                     for i in phases])
        ...
        def plot_phasespace(self):
                ...
                ph_min = np.array(
                    np.where(
                        self.ph_space['mat'] == self.ph_space['mat'].min())).transpose()
    ...
    """
        ...
        def _phase_neg_area_mp(self, thresh=0.0, ):
                ...
                proc_pool = Pool(cpu_count()-1)
                self.data = np.array(
                    proc_pool.map(_unwrap_fid_neg_area,
                                  zip([self] * len(self.data), range(len(self.data)))))
                proc_pool.close()
                proc_pool.join()
        ...
        def _phase_neg_area(self, thresh=0.0, ):
                ...
                def err_ps(p0, data):
                        err = self.ps(data, p0[0], p0[1], inv=False).real
                        err = np.array(
                            2 * [abs(err).sum() + abs(err[err < thresh]).sum()])
                        # np.array([abs(err).sum(),abs(err[err<thresh]).sum()])
                        return err
                if len(data.shape) == 2:
                        for i in range(data.shape[0]):
                                p0 = [0, 0]
                                ...
                                if abs(data_ph[i].min()) > abs(data_ph[i].max()):
                                        data_ph[i] = -data_ph[i]
        ...
        def _convert_peaklist_to_ppm(self):
                ...
                self.ranges = [
                    self.conv_to_ppm(
                        ...
                        i,
                        self.params['sw_left'],
                        self.params['sw']) for i in self.ranges]
        ...
        def plot_deconv(
                ...
                def i2ppm(index_value):
                        return np.mgrid[
                            sw_left -
                            self.params['sw']:sw_left:complex(
                                len(data))][index_value]
                ...
                for irange in paramarray:
                        for ipeak in irange:
                                ...
                                if txt:
                                        text(i2ppm(int(ipeak[0])),
                                             0.1+pk.max(),
                                             str(peaknum(paramarray,
                                                         ipeak)),
                                             color='#336699')
                ...
                ax.set_position([box1.x0,
                                 box1.y0+0.05*box1.height,
                                 box1.width,
                                 box1.height * 0.95])
        ...
        def plot_dict(
                ...
                if not cl:
                        cl = dict(
                            zip(d, plt.cm.Set1(np.linspace(0, 1, len(d)))))
                if not fmt:
        ...
        def plot_integrals(self, index='all'):
                ...
                self.integral_dict = dict(
                    zip(['%0.3f' % i for i in self.peaks],
                        self.integrals.transpose()))
                ...
                if ranges is not None:
                        ranges = [
                            self.conv_to_index(
                                ...
                                r,
                                self.params['sw_left'],
                                self.params['sw']) for r in ranges]
                return np.array([[sum(j[::-1][i[0]:i[1]])
                                for i in ranges] for j in self.data])
        ...
        def f_splines(self, s=1000, k=3):
                ...
                if len(self.data.shape) == 1:
                        ...
                        sys.stdout.flush()
                        return
                if len(self.t) is not len(self.data):
                        self.t = self.t[:len(self.data)]
                x = self.t
                splines = []
                rates = []
                concs = []
                for i in range(len(self.integrals_sum)):
                        if isinstance(k, int) is True:
                                spln = UnivariateSpline(
                                    x,
                                    self.integrals_sum[i],
                                    s=s,
                                    k=k)
                        elif isinstance(k, int) is False:
                                spln = UnivariateSpline(
                                    x,
                                    self.integrals_sum[i],
                                    s=s,
        ...
        def f_plot_splines(
                ...
                cl = cm.Paired(
                    np.mgrid[0: 1: np.complex(len(self.integrals_sum))])
                ...
                ax1.set_position([box1.x0,
                                  box2.y0+0.05*box1.height,
                                  box1.width * 0.85,
                                  box1.height * 0.95])
                ax2.set_position([box1.x0 +
                                  1.1 *
                                  box1.width, box2.y0 +
                                  0.05 *
...
def f_pk(p, x):
        ...
        f = p[-1]*fgss(p[np.array([0, 1, 2])], x)+(1-p[-1]) * \
            flor(p[np.array([0, 3, 4])], x)
        return f
...
#===================================
        ...
        for i in p:
                if i[-1] > 1:
                        # this constrains the lor/gauss ratio <= 1
                        err += (i[-1]-1)
        return res*(1+err)
...
#       init_ref = np.where(data_convolution == data_convolution_max)[0][0]/-len(data)
        init_ref = np.where(data_convolution == data_convolution.max())[0][
            0]-np.where(auto_convolution == auto_convolution.max())[0][0]
        return init_ref
        ...
        def press(self, event):
                ...
                if tb.mode == '':
                        ...
                        if event.inaxes is not None and (x >= 0) and (x <= len(self.data)-1) and event.button == 1:
                                # test for previous inclusion of current datum
                                if 1*(np.array(self.xs)[np.array(self.xs) == x]).sum() == 0:
...
class Phaser(object):
        ...
        def onmove(self, event):
                ...
                self.y = y
                if self.button == 1:
                        self.phases[0] = self.phases[
                            0] + 50*dy/self.ax.get_ylim()[1]
                if self.button == 3:
                        self.phases[1] = self.phases[
                            1] + 50*dy/self.ax.get_ylim()[1]
                self.datanew = self.ps(
                    self.data,
                    p0=self.phases[0],
                    p1=self.phases[1])
                self.ax.lines[0].set_data(
                    np.array([np.arange(len(self.datanew)), self.datanew]))
...
class SpanSelector:
        ...
        def __init__(self, data, params):
                ...
                ppm = np.mgrid[
                    params['sw_left'] -
                    params['sw']:params['sw_left']:complex(
                        len(data))]
                # color='#3D3D99', lw=1)
                self.ax.plot(ppm[::-1], data, color='0.3', lw=1)
                ...
                self.peaks = []
                self.ylims = np.array(
                    [self.ax.get_ylim()[0], self.data.max() +
                     abs(self.ax.get_ylim()[0])])
